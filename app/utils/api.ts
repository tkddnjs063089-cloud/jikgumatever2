...
export function getApiBaseUrl(): string {
  ...
  if (!apiBaseUrl || apiBaseUrl.trim() === "") {
    throw new Error("NEXT_PUBLIC_API_BASE_URL 환경 변수가 설정되지 않았습니다. .env.local 파일을 확인해주세요.");
  }
...
export async function apiCall(endpoint: string, options: RequestInit = {}): Promise<Response> {
  ...
  if (endpoint.includes("/users/") || endpoint.includes("/products/") || endpoint.includes("/auth/logout")) {
    try {
